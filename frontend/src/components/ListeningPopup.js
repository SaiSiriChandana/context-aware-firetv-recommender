...
const SpeechPopup = ({ onClose, onTranscriptComplete }) => {
  ...
  const sendToAssemblyAI = async (audioBlob) => {
    ...
    try {
      const uploadRes = await fetch('http://localhost:5000/api/assemblyai/upload', {
        method: 'POST',
        body: audioBlob,
      });
      ...
      const transcriptRes = await fetch('http://localhost:5000/api/assemblyai/transcript', {
        method: 'POST',
        headers: {
      ...
      let completed = false;
      while (!completed) {
        const polling = await fetch(`http://localhost:5000/api/assemblyai/transcript/${id}`);
