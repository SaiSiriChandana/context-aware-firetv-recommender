...
@app.route('/api/assemblyai/upload', methods=['POST'])
def assemblyai_upload():
...
@app.route('/api/assemblyai/transcript', methods=['POST'])
def assemblyai_transcript():
    api_key = os.getenv('ASSEMBLYAI_API_KEY')
    if not api_key:
        return jsonify({'error': 'API key not set'}), 500
...
@app.route('/api/assemblyai/transcript/<transcript_id>', methods=['GET'])
def assemblyai_poll(transcript_id):
    api_key = os.getenv('ASSEMBLYAI_API_KEY')
    if not api_key:
        return jsonify({'error': 'API key not set'}), 500
